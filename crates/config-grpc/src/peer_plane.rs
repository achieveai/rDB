...
impl PeerSvc {
    ...
    async fn call(
        ...
        self.check_transport_identity(&request, cluster_id, from)?;

        let env = request.into_inner();
        let req: PeerRequest = serde_json::from_slice(&env.payload)
            .map_err(|e| Status::invalid_argument(format!("undecodable peer payload: {e}")))?;
        ...
        Ok(Response::new(pb::PeerEnvelope {
            ...
            // Identity fields are echoed with the direction swapped: the answer travels from
            // us back to the caller.
            from_node_id: to.0,
            to_node_id: from.0,
