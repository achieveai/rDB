...
Every one of the following removes admission rights immediately and puts the node in a terminal
self-fenced state that only a **new grant id** can leave:

| Trigger | Spec source |
|---|---|
| clock error beyond the configured bound, or no bound established | §7.2 |
| backward clock jump | §7.2 |
| process resume after a suspension the scheduler observed | §7.2 |
| reboot / boot UUID change | §7.2 |
| authority-generation change | §7.2 |
| grant record frozen, revoked, absent, or owned by a different grant id | §7.2, §7.3 |
| our conservative expiry crossed without a committed renewal | §7.2 |
| local storage failure (partition-scoped fence only) | §5.2 step 3 |
...
Bounded-clock mode configures verified maximum UTC error **ε = 100 ms** and dispatch margin
**δ = 100 ms** (§7.2).
...
| Clock-bound violation fails closed | sample with `valid = false`, a stale sample, and a backward jump: all three deny with `ClockUnbounded` and self-fence |
