...
| M3-08 | peer_plaintext_connection_rejected | a raw TCP/h2 client with no TLS | dial the peer port and send a `PeerService` request | connection refused/closed at TLS; no `PeerService` handler runs; no panic; the server keeps serving valid peers afterwards | zero `@logger LIKE 'config_grpc::peer%'` request lines for that connection Lead note, 2026-09-19 M6 gate: the vacuity guard (at least one peer-plane line since the baseline) failed once under `--test-threads=2`: the acceptor refuses the probe in milliseconds, which is shorter than a heartbeat interval, so the window can hold nothing yet. The row now waits for the next legitimate peer-plane line with `cluster.wait_for` before the attribution and non-empty checks; the claims are unchanged. |
