...
| M5-71 | `m5_71_a_v1_dir_with_an_undrained_log_never_joins` (config-testkit) | `provision_seeded_dir(seed)`; start it as a learner against a v2 cluster | the node performs the bounded v1→v2 migration only if it is a *fresh* store per ADR-0021; a v1 store carrying data is refused with a typed format error naming `format_version`; it never joins with a silently-migrated journal | §17 "State format migrations are explicit, forward-tested, backed up, and have a documented rollback boundary"; D4.1 |
