...
`design.md round 4 (B-R32)` (provisional row; re-checked when the round-4 design text lands, see §9).
...
| M7B-57 | `m7b_57_differs_at_head_is_divergence_and_sends_nothing` | D §3.6 `Differs` arm "emit `DivergenceDetected(copy)` and nothing else" (K-B-45, B-R31); 0005 §6 | `NeedPrefix{from 11, head_digest ≠ ladder[10]}` retained | cursor effects `== [DivergenceDetected(B)]` (the rest of the vector is the tracker's, M7B-140); no `SendEnvelopes`; that peer's stream quarantined in the cursor; twin of M7B-55 by digest only, of M7B-56 by retention only | unit | none |
...
## 9. Round 3 rows (former holds, M7B-120..137; round-3 findings M7B-138..142; B-R32 provisional M7B-143..144; K-B-49 M7B-145)
...
M7B-143 and M7B-144 are **provisional, citing ruling B-R32** (architect round 4, K-B-51/K-B-52);
they are written against the ruling text and re-checked against design.md when round 4 lands.
...
| M7B-124 | H5 (K-B-37) | `m7b_124_recovered_behind_the_cutoff_keeps_applied_and_asks_for_the_gap` | D §3.3 behind-the-cutoff paragraph + `NotRetained` arm; 0005 §3 | receiver at `(50, d50)`, `Recovered{cutoff 100, ..}` | `applied_head == (50, d50)`; `accept_head == applied_head`; `history_floor == 100`; `received/buffered == 50`, not 100; effects contain `NeedPrefix{from 51, head_digest d50}`; then M7B-123's path fills 51..100 | unit | none |
...
| M7B-142 | new (K-B-46) | `m7b_142_block_partition_is_l1s_fourth_input_and_reads_as_a_block` | D §1.4 L1 inputs; §4.1 `blocked` one writer, never cleared; §4.4 `on BlockPartition` arm; §4.5 `reason`; 0006 §4 arm + "A block is not a pause" + row "A block reads as a block"; D §7 row "BlockPartition before the first Gained" | (a) fresh instance (`Paused` per M7B-141), `BlockPartition{DivergenceRequiresOperator, [C, B]}` with no prior `Lost`; (b) `Healthy` instance, same event | (a) mode stays `Paused` (no second `Paused` entry), `blocked == Some(DivergenceRequiresOperator)`, effects `== [SetAdmission(Reject(DIVERGENCE_REQUIRES_OPERATOR))]`, `AdmissionState.reason == DIVERGENCE_REQUIRES_OPERATOR`, `lost_copies` unchanged by this event; (b) → `Paused{highest_applied}` + the same reject; in both, `HealthEval` x100 never clears `blocked`; a second `BlockPartition` is idempotent; P1's `PubMode::Blocked` is kernel-a's row (cross-reference, not asserted here) | unit | none |
| M7B-143 | provisional, B-R32 (K-B-51) | `m7b_143_paused_never_reprotects_while_blocked_even_after_config_change_and_gained` | B-R32: §4.4 `Paused → Reprotecting` gains `AND blocked.is_none()`; critic K-B-51 closure row; charter DO-NOT "no one-copy ACK fallback" | `Paused`, `blocked Some(DivergenceRequiresOperator)`; `ConfigChanged` adds regular D; D ACKs at head → `QualificationChanged{Gained}`; `DurableAdvanced` satisfies the barrier on every predicate; `HealthEval` | mode stays `Paused`; no `SetAdmission(Allow)` in any effect vector; `AdmissionState{allow false, reason DIVERGENCE_REQUIRES_OPERATOR}`; twin (one fact: `blocked None`) → `Reprotecting` per M7B-75 | unit | design.md round 4 (B-R32) |
| M7B-144 | provisional, B-R32 (K-B-52) | `m7b_144_copy_quarantined_reaches_the_tracker_and_stalls_rebuilding_loudly` | B-R32: tracker consumes `CopyQuarantined` exactly like `DivergenceDetected` (one writer); `Rebuilding` treats the resulting `CopyLost` as a stall; §3.3 `Differs` row drops "rebuild target"; critic K-B-52 closure row | `Recovered` on a `required` copy B hits the `Differs` arm (M7B-139 fixture) and emits no `ProgressAck`; cursor `QUARANTINED` outcome → `CopyQuarantined{B}`; routed to the tracker; then `DurableAt` from the other two copies | tracker step on `CopyQuarantined{B}` → `peers[B].diverged == true` and the B-R26 vector once (idempotent with a later `DivergenceDetected(B)`, M7B-140); `Rebuilding` receives `CopyLost{B}` → exactly one `Alert{RebuildStalled, B}`, `required` unchanged, no `ActivationProposed` on any later `DurableAt`; Q-56 grep shows a tracker arm for `CopyQuarantined` (no unconsumed emitter) | unit | design.md round 4 (B-R32) |
...
until then they report `Unavailable`, not pass. M7B-143/144 report `Unavailable{"design round 4"}`
until the lead confirms the B-R32 text in design.md.
...
| Q-56 | `rg -n "CopyQuarantined" crates/rdb-core/src` (provisional, B-R32) | at least one consuming `match` arm in `replication.rs` (tracker) besides the emitter in the cursor | M7B-144 |
...
| M7B-143, 144 | `blocked.is_none()` conjunct; tracker `CopyQuarantined` arm | architect round 4 (B-R32) | design.md round-4 text lands and the lead lifts the provisional marker |
...
- [ ] V3: M7B-92, 97, 111, 112, 126, 128, 136, 137, 138, 139, 140 pass; M7B-144 once B-R32 lands.
- [ ] V8: M7B-65, 66, 67, 68, 71, 75, 80, 129, 130, 131, 132, 141, 142 pass; M7B-143 once B-R32 lands.
- [ ] V12: Q-53, Q-54, Q-55 and the C0 additive check from the verification plan §13 pass.
...
| §9 round 3 (120–145: 18 former holds, 5 round-3 findings, 2 provisional B-R32, 1 K-B-49) | 24 | 2 | 0 | 26 |
| Active total | 134 | 11 | 0 | 145 |
| Q-rows (Q-46..Q-56; Q-56 provisional) | 11 | 0 | 0 | 11 |
