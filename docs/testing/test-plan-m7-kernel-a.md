...
| M7A-169 | `published_while_frozen_retains_dedup_in_both_orders` | ADR 0004 "Published while frozen retains dedup in both orders" · K-A-46 | orderings A and B of M7A-168, narrowed to the dedup effect, plus a retry of the same request identity after each | `RetainDedup{identity, digest, result}` is emitted exactly once per ordering, with a **byte-equal** payload between the two; the retry after either ordering replays the retained result verbatim, reserves no sequence and emits zero `StorageBatch`. Near-miss twin **— written against an open question, §13 Q-17, not against a settled rule (TD-19)**: a batch that completes `Err` while frozen ⇒ **no** `RetainDedup`, and the retry re-executes (one fact: the completion result). The "retry re-executes" half is in tension with `RdbError::UnknownOutcome`'s landed retry rule, `RetryRule::QueryStatus` — *"Query status with the same identity. Never generate a fresh request id"* (`errors.rs:45-66`) — because M7A-82 answers that same `Err` with `Reply(UNKNOWN_OUTCOME)`. The twin is the default the row is implemented against if the architect does not rule first; it is **not** settled fact, and round 4 stated it as one. Read Q-17 before implementing this twin | unit | K-A-46; **§13 Q-17 open** (provisional) |
...
| Every question routed to the architect is marked open **in this document**, not only in the handoff | §13 preamble; critic R3 TD-19 | Q-12, Q-15, Q-16, **Q-17** (M7A-169's `Err` twin); each row whose cell depends on one names it |
...
**Round 5 adds one.** It is open and **routed to the architect**; this plan does not settle it.

| # | Question | Default (what the row is written against) |
|---|---|---|
| Q-17 | M7A-169's near-miss twin says a batch completing **`Err` while frozen** retains no dedup entry **and the retry re-executes**. Does it? `RdbError::UnknownOutcome` carries `RetryRule::QueryStatus` — "Query status with the *same* identity. **Never generate a fresh request id**" (`errors.rs:45-66`) — and M7A-82 answers exactly this `Err` with `Reply(UNKNOWN_OUTCOME)`. A client told `UNKNOWN_OUTCOME` is instructed to query, not to resubmit, so "the retry re-executes" describes a client doing what the retry rule forbids; and if T1 does execute a resubmission under that identity it has no retained outcome to compare against and the mutation can land twice. Which governs — the dedup rule (no entry ⇒ fresh execution) or the retry rule (same identity ⇒ status query)? | **Open.** The twin is written against "no `RetainDedup`; a resubmission under the same identity re-executes", and that is what you implement if the architect does not rule first. Round 4 stated this as settled fact with no marking anywhere in the deliverable — the routing was recorded only in the handoff, so a developer implementing M7A-169 had no signal the twin was contested (TD-19). If the ruling goes to `QueryStatus` the twin changes by exactly one fact (the retry answers `Unknown` from status instead of re-executing); M7A-169's main assertion — the byte-equal `RetainDedup` across orderings A and B — is untouched either way, which is why this is one cell and not a re-plan. Rows: M7A-169 (twin only); M7A-82 unaffected |
