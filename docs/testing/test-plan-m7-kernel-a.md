...
**Status:** Proposed — test planner deliverable, **correction round 3** under lead ruling **A-R26**
(critic-kernel-a round 3 findings T-A-01..15 applied; §8.7 holds the rows round 3 added, §15 the
drift against landed C0)
**Date:** 2026-09-20
...
ADR verification rows as of `3eec5e9`; landed code C0 as of `8a23b1d`.
...
> (§13 Q-1). Drift between design round 3 and landed C0 at `8a23b1d` is recorded in §15.
...
(`Resolved(TxnResult) | Unresolved { seq } | Unknown | Expired`), and `txn::Outcome` at `8a23b1d`
has two unit variants that are **not** this enum. A row asserts on whichever side it names, using
this mapping, stated once:
...
`TxnStatus::Unresolved { seq }` is T1's answer for a frozen queue, not P1's; no M7A row produces
it from `StatusIndex`. §15 records the drift.
...
| M7A-101 | `exactly_one_reply_per_request_on_every_path` | §4.3 invariant 6 (K-A-13, re-stated across `Pending.replied` and `AwaitingReply.replied` under K-A-40) · K-A-45 paths (T-A-15) | six paths: (a) publish+reply; (b) publish then reply-deny; (c) `PostApplyDeadline` then late `Gained`+`Admit`; (d) quarantine; (e) **K-A-45**: `Candidate` arrives in `Frozen{AuthorityLost}`, then the partition is never served again; (f) **K-A-45**: `Candidate` arrives in `Blocked{reason}`, then `Recovered` | count of `Reply` effects for the identity == 1 on (a), **0** on (b), 1 on (c) and (d), **0** on (e) and (f) — the accepted-while-not-serving candidate answers through `Status(Unknown)`, never a second `Reply`; never 2 anywhere; `awaiting_reply` empty at the end of every path | unit | K-A-40, K-A-45 |
...
| M7A-124 | `fake_arbitrarily_late_completion_after_expiry` | ADR 0008 §7 item 7 | `PlanCas{outcome: Committed}` scheduled to land 5,000 ticks later. The landed `ControlOp::PlanCas` has **no delay member** (§15 drift row 5), so the lateness comes from the **sim scheduler's** delivery delay, not from a field on the op | the `CasResult` arrives after A1 fenced `Expired`; A1 answers `LateRenewalIgnored` (M7A-22) | unit | H1; §15 drift row 5 |
...
| M7A-135 | `f1_t1_p1_retention_boundary_recovered_applied_then_unknown` | spike §6 F1/T1/P1 · spec §8.1 · ADR 0004 retention boundary · §4.4 `fold_recovered` (K-A-52) | commit seq 5 in gen g; recover into g+1 with `RetainedStatusMap{retained_through: 5, discarded_from: None, uncertain: false}`; retry ⇒ `RECOVERED_APPLIED`; `RetireGeneration{g}`; retry again; a `DedupTrim` below 5 in a live gen instead | first answer `RecoveredApplied{result}`; after the retirement the identity is **absent**, so the answer is `Unknown`, **not** `StatusExpired` — in M7 nothing is ever discarded below `retained_through`, so `StatusExpired` is unreachable and the row asserts it is never produced on any of the three paths (K-A-52); `DedupTrim` path ⇒ `Unknown`; never a second execution of seq 5's mutation. Loss-accepting variant in the same test (one fact: `uncertain: true` on the recovery map) ⇒ the first retry answers `Unknown` instead of `RecoveredApplied` | sim | K-A-52; kernel-b F1 |
...
| M7A-166 | `every_fence_publishes_an_already_past_view` | ADR 0007 "Every fence publishes an already-past view" · K-A-49 · §1.7 "`fence()` writes the view directly, never calling `admission_horizon`" | iterate **every** fence row of ADR 0007 §3's table — node-scoped (`Expired`, `ClockUnbounded`, `LocalStorageFenced`) and partition-scoped (`GenerationChanged`, `ConfigVersionChanged`) — each fired at tick `t` with `served = {p1, p2}` | for every row: the view pushed with the fence has `valid_through_tick == t − 1` and `past_horizon == Some(<that fence's reason>)`, and `authority_seq` bumped. **No fence anywhere in the table produces a view whose `valid_through_tick >= t`**, and none produces `past_horizon == None`. Partition-scoped rows push only for the fenced partition; node-scoped rows push for both (M7A-145 counts the fan-out) | unit | K-A-49 (provisional) |
...
| M7A-172 | `recovery_folds_status_by_sequence_not_by_presence` | ADR 0004 "Recovery folds status by sequence, not by presence" · K-A-52 · §4.4 `fold_recovered` | direct table test of `fold_recovered` over the three-way rule: for `RetainedStatusMap{retained_through: 10, discarded_from: Some(11), uncertain: false}`, query `seq` ∈ {9, 10, 11, 12}; then the same with `uncertain: true`; then `discarded_from: None` | `uncertain: false`: 9 and 10 ⇒ `RecoveredApplied{result}`; 11 and 12 ⇒ `Unknown` (`seq >= discarded_from`), **not** `RecoveredApplied`. `uncertain: true`: all four ⇒ `Unknown`. `discarded_from: None`: 9, 10 ⇒ `RecoveredApplied`, 11, 12 ⇒ `Unknown`. Across every cell `StatusExpired` is **never** returned — M7 keeps `retained_through` at or above every retained identity, so the third arm is unreachable and the row says so (M7A-116 is the kernel-level row; this is the fold in isolation) | unit | K-A-52 (provisional) |
...
| M7A-69(b), M7A-69(c) | **kernel-b** L1 `AdmissionState{allow, reason}` seam, **and** `ErrorKind::DivergenceRequiresOperator` | the variant does not exist in `rdb-core` today; it is a **dependency**, tracked as kernel-b **B-R31 item 18**, not a finding against this plan (T-A-09). (a) runs now |
| M7A-116, M7A-117, M7A-135, M7A-136, M7A-172 | **kernel-b** F1 recovery event carrying `RetainedStatusMap{retained_through, discarded_from, uncertain}` (K-A-52) | recovery fold and reconciliation; M7A-172 tests the fold in isolation and needs only the type |
...
| Every ADR 0004 verification row (15 at 785e41b, **3 added at `3eec5e9`** = 18; "Sequence reservation is discardable" amended, §3 row 8 amended for the `AdmissionState.reason` passthrough, the `DIVERGENCE_REQUIRES_OPERATOR` §5 row amended) | ADR 0004 | 1→72 · 2→73 · 3→76 · 4→84+85 · 5→62/63 · 6→78 · 7→79 · 8→113 · 9→74 · 10→77 · 11→80+138+82 · 12→87 · 13→90 · 14→117/136 · 15→83 · **Recovery folds status by sequence, not by presence**→172+116+135 · **Publication binds the digest**→173+93 · **Published while frozen retains dedup in both orders**→169+168; §3 row 8 passthrough→69 · `DIVERGENCE_REQUIRES_OPERATOR`→69(c)+71 |
...
| Design drift against landed C0 at `8a23b1d` recorded, not silently fixed | critic R3 T-A-11 | §15 |
...
|---|---|---|
| Q-12 | KA-1 says kernel-a's time arrives as its own `AuthorityEvent::Tick(u64)` and `AuthorityEvent::Clock(ClockSample{at, utc_ms, epsilon_ms, valid})`, but landed C0 carries time as `StepCtx.now` plus `ControlTime{estimate, error_millis, bound_established}`. Who converts, and on what rule? | **I1 in `rdb-sim` builds the `ClockSample` from the landed `ControlTime`**, with `valid = ct.bound_established` **and nothing else**; samples are delivered **even when the bound is not established** (so A1 sees the invalid sample and fences, rather than seeing nothing); **no staleness filtering at the seam** — staleness is A1's judgement, against `max_sample_age_ticks`. This is foundation contract request 9 from kernel-a (§15 drift row 7). Every row that feeds a clock sample depends on it: M7A-38..M7A-46, M7A-143, M7A-146, M7A-148, M7A-165 |
...
- Landed code is C0 at `8a23b1d`. Where design round 3 and landed C0 disagree, **§15 records the
  drift**; this plan does not resolve it and does not edit `design.md` or the ADRs.
...
## 15. Drift: design round 3 versus landed C0 at `8a23b1d` (T-A-11)
...
| # | Subject | Design round 3 | Landed C0 at `8a23b1d` | How the rows cope |
|---|---|---|---|---|
...
| 4 | Read reply | F-R7 asserts `ReplyEffect::Read`; design §4.2 reads return a snapshot handle | `event.rs` shows `Transaction, Status, Failed` — **no `Read`**; and `SnapshotHandle(u64)`, **no `SnapshotId::at`** | §13 Q-9: rows are written against `Read`/`SnapshotId::at` and listed in §11 as `unavailable until C0`, never rewritten around `Transaction` |
| 5 | Control op delay | ADR 0008 §7 item 7 needs an arbitrarily late completion | `ControlOp::PlanCas{outcome}` has **no delay member** | M7A-124 takes the lateness from the **sim scheduler's** delivery delay, not from a field on the op. Item 8 (never completes) still needs `ControlOp::DropNext` (§13 Q-8) |
| 6 | Checkpoint spelling | Q-42's log field and §2.5 name the admission checkpoint in prose | `AuthorityGate::Dispatch` | Q-42's query matches the landed spelling; if the gate is renamed the query string moves with it, not the row |
| 7 | Clock sample at the seam | §1.7 and §2.3 consume `ClockSample{at, utc_ms, epsilon_ms, valid}` | time arrives as `StepCtx.now` plus `ControlTime{estimate, error_millis, bound_established}` | **Foundation contract request 9 from kernel-a**: I1 in `rdb-sim` builds the sample, `valid = ct.bound_established` and nothing else, samples delivered **even when the bound is not established**, **no staleness filtering at the seam**. §13 Q-12; the clock rows (M7A-38..46, 143, 146, 148, 165) depend on it |
| 8 | Read outcome | spec §5.2 shorthand `ReadOutcome::Found{revision, value}` | the landed read path returns its own shape | M7A-107..M7A-111 assert the fields (`revision`, `value`), not the constructor spelling, so a rename does not re-word them |

None of these eight is a reason to lower an assertion (hard rule "never lower an assertion"). Rows
