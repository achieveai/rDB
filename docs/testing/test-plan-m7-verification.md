...
`TraceHeader` at `ec610f4` (`schema_version`, `generator_version`, `provenance: Provenance` —
...
**Verification has zero open contract asks on foundation** (re-read at `ec610f4`, §15 drift table).
...
**Drift table — `trace-requirements.md` §3 as this plan cited it vs. the landed C0 at `ec610f4`
(critic T-23; re-read in full 2026-09-20, round 5).** Read from
`git show ec610f4:crates/rdb-core/src/contracts/{trace,ids,errors}.rs`, not the working tree.
...
**The basis moved twice since this table was first written, and the whole table was re-read
against the newer one, not just the rows that were flagged.** `8a23b1d` → `6893442` (foundation's
first code round) → `ec610f4` (K-F-39). Six rows had moved: **1** and **21** landed and are no
longer asks; **6**, **8** and **10** gained or changed fields; **12** landed the withdrawal it
recorded as refused. The rest were re-read and still hold. **Verification's open-ask count is now
zero** — the marker `<!-- drift-basis: ec610f4 -->` below is set on the strength of that re-read
and on nothing else. A marker set without it silences `scripts/drift-check.sh` instead of
satisfying it, which is worse than the stale table it replaces.

| # | Field, as the plan wrote it | Landed shape at `ec610f4` | Disposition | Rows changed |
|---|---|---|---|---|
...
One contract change is **expected and welcome**: kernel-b's CB-3 widens `AckRejectReason` by seven
variants (ruling **B-R33 Q-B-8**). `M7V-56` asserts **set equality** between the enum and the
coverage lists, so it will go red the moment the widening lands. That is the row working, not the
row breaking — **lead ruling F-R20 (2026-09-20 23:08 PDT) is explicit that `M7V-56` is not to be
weakened to a subset assertion**; the response is seven new coverage cells. A set-equality
assertion that degrades to subset the first time it fires was never an assertion. If the seven
cells cost materially more than expected, verification brings it back to the lead rather than
touching the row. Re-read at `ec610f4`, `AckRejectReason` carries exactly the seven names
`trace-requirements.md` §3.5 declares (`Gap`, `DigestMismatch`, `StaleEpoch`, `StaleBoot`,
`StaleConfig`, `ForgedIdentity`, `IncompatibleVersion`), so the tripwire is armed and correct
today. `BoundaryId` is still **29** members at `ec610f4`, so `M7V-56`'s other set-equality half is
unchanged.
